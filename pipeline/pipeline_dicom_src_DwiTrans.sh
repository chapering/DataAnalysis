...
if ! test -d ${name}/tube;then
	...
	if [ $defaultgen -ge 1 ];then
		...
		: '
		nohup tubegen -out ${name}/tube/${name}_1.sm -dt ${name}/tensors -t2 ${name}/I0 \
			-ODE 0 -cs 10 \
			-seed 5 5 5 -stepsize 1.0 -radius 0.3 -al 0.1 -dth .5 -t2thresh \
		...
		nohup tubegen -out ${name}/tube/${name}_2.sm -dt ${name}/tensors -t2 ${name}/I0 \
			-ODE 0 -cs 10 \
			-seed 1 1 1 -stepsize 1.0 -radius 0.3 -al 0.1 -dth .25 -t2thresh \
			-60 -Longshort 1 -cl .1 -cd 1.5 -cr 0 \
			 1>> ${name}/${name}.nohup.out 2>&1 < /dev/null &

			 wait

		nohup tubegen -out ${name}/tube/${name}_3.sm -dt ${name}/tensors -t2 ${name}/I0 \
